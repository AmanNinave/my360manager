...
const IncomeExpenditureBySourceChart = ({ transactionsData }) => {
  ...
  useEffect(() => {
    ...
    const incomeData = Array.from(
      d3.group(transactionsData, (d) => d.source),
      ([key, values]) => ({
        source: key,
    ...
    const xAxis = incomeSvg
      .append("g")
      .attr("class", "x-axis")
      .attr("transform", `translate(0,${incomeHeight})`)
      .call(d3.axisBottom(incomeX));

    xAxis.selectAll("text").style("opacity", 0); // Hide all labels initially

    // y ticks values
    const maxIncome = d3.max(incomeData, (d) => d.totalIncome);
    const incomeTicks = Array.from(
      { length: 10 },
      (_, i) => Math.ceil(maxIncome / 10000) * 1000 * (i + 1)
    ).filter((val) => val <= maxIncome);

    incomeSvg
      .append("g")
      .attr("class", "y-axis")
      .call(
        d3.axisLeft(incomeY).tickValues(incomeTicks).tickFormat(formatNumber)
    ...
    incomeSvg
      ...
      .on("mouseover", (event, d) => {
        ...
        xAxis.selectAll("text").style("opacity", 0); // Hide all labels
    ...
    const expenditureContainerWidth =
      expenditureContainerRef.current.clientWidth;
    const expenditureContainerHeight =
      expenditureContainerRef.current.clientHeight;

    const expenditureData = Array.from(
      d3.group(transactionsData, (d) => d.source),
      ([key, values]) => ({
        source: key,
    ...
    const expenditureTicks = Array.from(
      { length: 10 },
      (_, i) => Math.ceil(maxExpenditure / 10000) * 1000 * (i + 1)
    ).filter((val) => val <= maxExpenditure);
    ...
    expenditureSvg
      ...
      .call(
        d3
          .axisLeft(expenditureY)
          .tickValues(expenditureTicks)
          .tickFormat(formatNumber)
      ); // Specify the tick values

    // Add bars with tooltip on hover
    expenditureSvg
      .selectAll(".bar-expenditure")
      ...
      .attr(
        "height",
        (d) => expenditureHeight - expenditureY(d.totalExpenditure)
      )
      .attr("fill", "blue")
      .on("mouseover", (event, d) => {
        tooltip
        ...
        // Show only the label for the hovered bar
        expenditureXAxis.selectAll("text").style("opacity", 0); // Hide all labels
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ width: "45%", height: "90%", position: "relative" }}
      >
